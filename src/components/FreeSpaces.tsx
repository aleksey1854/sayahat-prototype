      ...
      <div className="wrap">
        <div className="arenda">
          <div className="arenda__txt">
            <h2>{t("Откройте свой бизнес в «Саяхате»", "«Саяхатта» өз бизнесіңізді ашыңыз")}</h2>
            <p>
              {t(
            ...
            <div className="arenda__price">
              <b>
                {/* Неразрывные пробелы расставлены по правилам русской
                    типографики: после короткого предлога, перед тире и
                    между числом и единицей. Без них на телефоне строка
                    рвалась между «за» и «1 м²», и вторая строка
                    начиналась с числа — это и выглядело криво. */}
                {t(
                  `Стоимость аренды${NB}— ${site.rentFrom} за${NB}1${NB}м² в${NB}месяц.`,
                  `Жалдау құны${NB}— айына 1${NB}м² үшін ${site.rentFromKz}.`,
                )}
              </b>
              <span>
                {t(
                  `Итоговая цена зависит от${NB}площади, расположения и${NB}оснащения места.`,
          ...
          <div className="arenda__cta">
            <a className="btn btn--accent btn--lg" href={telHref(site.rentPhone)} data-goal="rent_call">
            ...
            <a className="btn btn--ghost btn--lg" href={waLink(site.rentWhatsapp, waText)} target="_blank" rel="noopener" data-goal="rent_whatsapp">
              <WhatsAppIcon />
