 ...
 * Неразрывный пробел.
 *
 * Записан кодом, а не символом: в исходнике он неотличим от обычного
 * пробела, и при первой же правке текста молча превращается в обычный.
 * Кодом видно, что пробел особый.
 *
 * Ставится там, где перенос строки ломает смысл: внутри числа, между
 * числом и единицей, после коротких предлогов и перед тире.
 */
export const NB = "\u00A0";
...
export function price(value: number) {
  // Пробел перед знаком тенге неразрывный: иначе на узкой карточке
  // «1 500 ₸» разъезжается на «1 500» и одинокий «₸» строкой ниже.
  return `${tenge.format(value)}${NB}\u20B8`;
}
