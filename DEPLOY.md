...
> **База:** проект работает на PostgreSQL. Проще всего завести бесплатную базу на Neon (neon.tech), скопировать строки подключения в `.env` (`DATABASE_URL` и `DATABASE_URL_UNPOOLED`) и выполнить `npx prisma db push`. Инструкция ниже писалась под SQLite-файл — вместо копирования `prod.db` используй Postgres + `db push`.
